...
void KeccakTreeCPU(unsigned* inBuffer, unsigned* outBuffer)
{
    ...
    for (blkIdx = 0; blkIdx < NB_THREADS_BLOCKS; blkIdx++)  // loop on threads
                                                            // blocks
    {
                                                         ...
                                                         // inside a threadblock
        {
            ...
            for (k = 0; k < NB_INPUT_BLOCK; k++)
            {
                ...
                for (ind_word = 0; ind_word < INPUT_BLOCK_SIZE_B / 4;
                     ind_word++)
                {
                    Kstate[ind_word] ^=
                        inBuffer[thrIdx + ind_word * NB_THREADS +
                                 k * NB_THREADS * INPUT_BLOCK_SIZE_B / 4 +
                                 blkIdx * NB_THREADS * INPUT_BLOCK_SIZE_B / 4 *
                                     NB_INPUT_BLOCK];
                }
            ...
            for (ind_word = 0; ind_word < OUTPUT_BLOCK_SIZE_B / 4; ind_word++)
            {
                ...
                outBuffer[thrIdx + ind_word * NB_THREADS +
                          blkIdx * NB_THREADS * OUTPUT_BLOCK_SIZE_B / 4] =
                    Kstate[ind_word];
            }
...
void KeccakTreeCPU_2stg(unsigned* inBuffer, unsigned* outBuffer)
{
                                                            ...
                                                            // blocks
    {
                                                         ...
                                                         // inside a threadblock
        {
            ...
            {
                // printf("Kstate[%02u] = %08x",ind_word,Kstate[ind_word] );
                SharedBuffer[thrIdx + ind_word * NB_THREADS] = Kstate[ind_word];
            }
            ...
            {
                // printf("Kstate[%02u] = %08x",ind_word,Kstate[ind_word] );
                SharedBuffer[thrIdx + ind_word * NB_THREADS +
                             NB_THREADS * OUTPUT_BLOCK_SIZE_B / 4] =
                    Kstate[ind_word];
            }
             ...
             thrIdx++)  // 2nd loop on threads inside a threadblock
        {
            ...
            for (k = 0; k < NB_INPUT_BLOCK_SNCD_STAGE; k++)
            {
                ...
                {
                    Kstate[ind_word] ^=
                        SharedBuffer[thrIdx + ind_word * NB_SCND_STAGE_THREADS +
            ...
            {
                // printf("Kstate[%02u] = %08x",ind_word,Kstate[ind_word] );
                outBuffer[thrIdx + ind_word * NB_SCND_STAGE_THREADS +
                          blkIdx * NB_SCND_STAGE_THREADS * 2 *
