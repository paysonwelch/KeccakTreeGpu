...
const unsigned int KeccakF_RotationConstants[25] = {
    ...
    // 62, 18, 39, 61, 20, 44
    1, 3, 6, 10, 15, 21, 28, 4, 13, 23, 2, 14, 27, 9, 24, 8, 25, 11, 30, 18, 7,
    29, 20, 12};

// INFO It could be more optimized to use unsigned char on an 8-bit CPU
const unsigned int KeccakF_PiLane[25] = {10, 7, 11, 17, 18, 3, 5, 16, 8, 21, 24,
    ...
    for (round = 0; round < cKeccakNumberOfRounds; ++round)
    {
        ...
        for (x = 0; x < 5; ++x)  // derouler
        {
            temp = BC[KeccakF_Mod5[x + 4]] ^
                   ROL32(BC[KeccakF_Mod5[x + 1]], 1);  // expliciter
        ...
        for (x = 0; x < 24; ++x)  // expliciter + rotation modulo 32
        {
            ...
            state[KeccakF_PiLane[x]] =
                ROL32(temp, KeccakF_RotationConstants[x]);
            temp = BC[0];
        }
        ...
        for (y = 0; y < 25; y += 5)
        {
            ...
            for (x = 0; x < 5; ++x)
            {
                state[y + x] = BC[x] ^ ((~BC[KeccakF_Mod5[x + 1]]) &
                                           BC[KeccakF_Mod5[x + 2]]);
            }
        }
