								   ...
								   tKeccakLane * h_outBuffer, tKeccakLane * d_outBuffer)
{
	...
	for(s=0;s<NB_STREAMS;s++)
	{
		...
		ker_Keccak<<<NB_THREADS_BLOCKS/NB_STREAMS , NB_THREADS, 0, stream[s]>>>(d_inBuffer + s*inOffset ,d_outBuffer + s*outOffset);
	}
	checkCUDAError(" ker_keccak");
								   ...
								   tKeccakLane *Kstate)
{
	...
	for(s=0;s<NB_STREAMS;s++)
	{
		...
		ker_Keccak_2stg<<<NB_THREADS_BLOCKS/NB_STREAMS , NB_THREADS, 0, stream[s]>>>(d_inBuffer + s*inOffset ,d_outBuffer + s*outOffset);
	}
								   ...
								   tKeccakLane * h_outBuffer, tKeccakLane * d_outBuffer)
{
	...
	for(s=0;s<NB_STREAMS;s++)
	{
		ker_Keccak_SCipher<<<NB_THREADS_BLOCKS/NB_STREAMS , NB_THREADS, 0, stream[s]>>>(d_inKeyNonce ,d_outBuffer + s*outOffset);
